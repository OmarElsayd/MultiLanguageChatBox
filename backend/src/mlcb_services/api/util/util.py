...
from mlcb_services.db_models.models import MlcbUsers, MlcbSessions, UsersSession, Transcripts
from mlcb_services.dependency.exception_handler import (
    TranslationException, DbSessionException, raise_http_exception
)
from mlcb_services.util.constant import SUPPORTED_LANGS, TRANSCRIPTS_BODY
from mlcb_services.util.payloads import SessionPayload, InComingWsMessagePayload

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("API UTIL LOGGING")


def verify_session_invitation(user: MlcbUsers, db_session: Session, session_code: str):
    """
...
async def update_transcript_body_text(
    ...
    try:
        transcript_body_tuple = session.query(Transcripts).join(
            MlcbSessions, MlcbSessions.transcript_id == Transcripts.id
        ).filter(
            MlcbSessions.session_code == session_code
        ).one()
...
def init_transcript_body(session_payload: SessionPayload, user: MlcbUsers) -> dict:
...
async def verify_invitee(db_session, session_payload, user):
    invitee = (
        db_session.query(MlcbUsers).filter_by(
...
async def end_session(session_code: str, db_session: Session) :
    _session = db_session.query(MlcbSessions).filter(
        MlcbSessions.session_code == session_code
    ).first()
    if not _session:
        raise HTTPException(
